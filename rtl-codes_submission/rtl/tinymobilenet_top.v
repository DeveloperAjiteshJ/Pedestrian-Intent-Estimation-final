...
// UART back protocol removed — uart_tx held idle (1'b1)
// ============================================================================
...
`endif
    ...
    // UART TX held idle — back protocol removed
    assign uart_tx = 1'b1;
...
`endif
    ...
    always @(*) begin
        ...
        case (state)
            ...
            OUTPUT:      next_state = WAIT_BUTTON;
            default:     next_state = IDLE;
        endcase
    end

    // -------------------------------------------------------------------------
    // Sample streaming — BRAM -> inference core
    // -------------------------------------------------------------------------
    always @(posedge clk or negedge rst_n_int) begin
        if (!rst_n_int) begin
            rom_ptr    <= 16'd0;
    ...
    // -------------------------------------------------------------------------
    // Sample counter, accuracy tracker and display register update
    // -------------------------------------------------------------------------
    always @(posedge clk or negedge rst_n_int) begin
        if (!rst_n_int) begin
            sample_count             <= 16'd0;
            ...
            if (state == OUTPUT) begin
                ...
                if (sample_count != 16'd0)
                    accuracy_pct <= (((predicted_class == expected_class) ?
                                      (correct_count + 16'd1) : correct_count) * 8'd100)
                                    / (sample_count + 16'd1);
                else
                    accuracy_pct <= 8'd0;
    ...
    // -------------------------------------------------------------------------
    // Seven-segment anode / digit-value MUX
    // Display layout (right to left):
    //   [7]=Expected  [6]=E/C match  [5]=Predicted  [4]=blank
    //   [3]=Conf tens [2]=Conf ones  [1]=blank       [0]=cycle counter
    // -------------------------------------------------------------------------
    always @(*) begin
        an = 8'b1111_1111;
        dp = 1'b1;
        case (scan_sel)
            3'd0: begin an = 8'b1111_1110; digit_val = cycle_counter[3:0]; end
            3'd1: begin an = 8'b1111_1101; digit_val = 4'hB; end
            3'd2: begin an = 8'b1111_1011; digit_val = ((displayed_confidence > 8'd99) ? 8'd99 : displayed_confidence) % 10; end
            3'd3: begin an = 8'b1111_0111; digit_val = ((displayed_confidence > 8'd99) ? 8'd99 : displayed_confidence) / 10; end
            3'd4: begin an = 8'b1110_1111; digit_val = 4'hB; end
            3'd5: begin an = 8'b1101_1111; digit_val = displayed_pred_class ? 4'd1 : 4'd0; end
            3'd6: begin an = 8'b1011_1111; digit_val = (displayed_pred_class != displayed_expected_class) ? 4'hE : 4'hC; end
            3'd7: begin an = 8'b0111_1111; digit_val = displayed_expected_class ? 4'd1 : 4'd0; end
            default: begin an = 8'b1111_1111; digit_val = 4'hB; end
        endcase
    end

    // -------------------------------------------------------------------------
    // Seven-segment cathode decoder (common-anode, active-low segments)
    // -------------------------------------------------------------------------
    always @(*) begin
        case (digit_val)
            4'h0: seg = 7'b1000000;
...
// ============================================================================
// Inference Core  — streaming byte classification engine
// ============================================================================
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (finalize_pending) begin
                ...
                if      (margin > 32'd300000) confidence <= 8'd95;
                else if (margin > 32'd150000) confidence <= 8'd80;
                done <= 1'b1;
            end
        end
    end
...
// ============================================================================
// Tree Classifier  — combinational decision tree extracted from PyTorch model
// ============================================================================
...
// ============================================================================
// UART TX Module  — kept for reference; NOT instantiated in tinymobilenet_top
// ============================================================================
